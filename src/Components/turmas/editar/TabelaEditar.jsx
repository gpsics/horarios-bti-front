...
const TabelaEditar = ({ tur, numVagas, numTurma }) => {
    ...
    const updateTurma = async (e) => {
        ...
        Confirm.cadastrar().then(async (result) => {
            if (result.isConfirmed) {
                const url = `http://3.221.150.138:8000/api/turmas/${tur.id}/`
                const config = {
                    headers: {
    ...
    const lerHorariosTurmas = useCallback(async (identificador, tipo) => {
        let url = ''
        if (tipo) {
            url = `http://3.221.150.138:8000/api/horarios/semestre/${identificador}`;
        } else if (!tipo) {
            url = `http://3.221.150.138:8000/api/horarios/professores/${identificador}`;
        }
        const config = {
            headers: {
                Authorization: `Bearer ${token}`,
    ...
    const requisitarComponente = useCallback(async () => {
        ...
        const url = `http://3.221.150.138:8000/api/componentes/${tur.cod_componente}`;
        const config = {
            headers: {
                Authorization: `Bearer ${token}`,
            },
